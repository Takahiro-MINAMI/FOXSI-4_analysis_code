...
respfile=$SCRIPT_DIR"/gaploss_resp/resp_"$cdteid".root"
doirespfile=$SCRIPT_DIR"/doiloss_resp/doi_resp_"$cdteid".root"
poscalfile=$SCRIPT_DIR"/poscal_resp/position_20240514.root"
...
echo "poscalfile=$poscalfile" >> $logfile 2>&1
echo "doirespfile=$doirespfile" >> $logfile 2>&1
normal_line_gap

## set-up is done, now actually run the PATH scripts
echo "PATH 1" | tee -a $logfile 2>&1
echo $SCRIPT_DIR/path1_base_forwidegap_forcal $1 $2
normal_line $SCRIPT_DIR/path1_base_forwidegap_for_mergedevent $1 $2
normal_line_gap
...
normal_line_gap
echo "PATH 5" | tee -a $logfile 2>&1
normal_line $SCRIPT_DIR/path5_poscal $basefilename"_base_cal_gapmod_doimod.root" $poscalfile $2
normal_line_gap
...
normal_line mv $basefilename"_base_cal_gapmod_doimod_poscal.root" $directory"/path5/"$filename"_base_cal_gapmod_doimod_poscal.root"
